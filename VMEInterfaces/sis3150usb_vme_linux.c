...
/*     - in sis3150_vmebus_write: change long to uint32_t                     */
/*                                                                         */
/*                                                                         */
/*-------------------------------------------------------------------------*/
/*-------------------------------------------------------------------------*/
...
/*                                                                         */
/***************************************************************************/
//UNUSED:static char* copyright=" � 2006 SIS Struck Innovated Systeme GmbH All rights reserved\n";
...
static const int    productIdUsb3   = 0x3153;          /* Product ID we are looking for */
...
//UNUSED:static const char*  rootEnv      = "SIS3150";
//UNUSED:static const char*  firmwareName = "setup_8051.hex"; 
//UNUSED:static const char*  usbDeviceDir = "/proc/bus/usb/";

/*  Fields/bits in the request Space/CTL word */

static const uint32_t USB_HEADER     = 0xaaaa0000; /*  USB Request header. */
static const uint32_t SPACE_REGISTER = 1 << 12;
static const uint32_t SPACE_TSBUS    = 2 << 12;
static const uint32_t SPACE_VME      = 4 << 12;

static const uint32_t  CTL_WRITE        = 0x0800; /* Operation is a write. */
static const uint32_t  CTL_DISABLE_INCR = 0x0400; /* Don't autoincrement addresses */
static const uint32_t  CTL_INCR_BYTE    = 0x0000; /* Increment for bytes    */
static const uint32_t  CTL_INCR_SHORT   = 0x0100; /* Increment for 16 bit words */
static const uint32_t  CTL_INCR_LONG    = 0x0200; /* Increment for 32 bit words */
static const uint32_t  CTL_INCR_DOUBLE  = 0x0300; /* Increment for 64 bit words */
static const uint32_t  CTL_XFER_WIDTH   = 0x0700; /* Mask for transfer width field */

static const uint32_t  CTL_LISTMODE     = 0x0020; /* Op is list mode?           */
static const uint32_t  CTL_TSBUS_SGLFLG = 0x0002; /* ???                        */
static const uint32_t  CTL_TSBUS_D64    = 0x0001; /* 64 bit wide Tigersharc bus access */

static const uint32_t  AMOD_SHIFT       = 16;     /* left shift count for addr. mod. */

static const int   XFER_WRITE_HEADERSIZE = 3*sizeof(uint32_t); /* Bytes in write xfer header */
static const int   XFER_READ_HEADERSIZE  = 2*sizeof(uint32_t);
...
Sis3150usb_OpenDriver_And_Download_FX2_Setup(PCHAR usbDeviceName, 
					     struct SIS3150USB_Device_Struct* sis3150usb_Device)
{
  //UNUSED:char*  sis3150RootDir;	/* Where the software lives. */
  //UNUSED:char   firmwareFilename[PATH_MAX+1];
  //UNUSED:char   usbDeviceFile[PATH_MAX+1];
  //UNUSED:int    testFd;
  //UNUSED:struct stat info;
  //UNUSED:struct usb_device* hardwareHandle;
  //UNUSED:struct usb_dev_handle* loadHandle;
  //UNUSED:int             status;
...
//  strncat(usbDeviceFile, usbDeviceName, PATH_MAX);
  ...
  //status = fx2_load_firmware(firmwareFilename);
...
static int sis3150_register_read(HANDLE hDevice, uint32_t addr, uint32_t* data, 
                      uint32_t req_nof_lwords, uint32_t* got_nof_lwords)
{
  int return_code ;
  ...
  //UNUSED:BOOLEAN success;
  uint32_t usb_wlength;
  uint32_t usb_rlength;

  //UNUSED:UINT uiData = 0;
  char* cUsbBuf_ptr;
  //UNUSED:unsigned int* longUsbBuf_ptr;
  ...
  if(req_nof_lwords > USB_MAX_NOF_LWORDS) {
	...
	*got_nof_lwords = (uint32_t) (nLWords )  ;
...
int EXPORT sis3150Usb_Register_Single_Read(HANDLE usbDevice, uint32_t addr, uint32_t* data)
{
  int return_code ;
  uint32_t req_nof_lwords, dma_got_nof_lwords;
...
int EXPORT sis3150Usb_Register_Dma_Read(HANDLE usbDevice, uint32_t addr, uint32_t* dmabufs, uint32_t req_nof_data, uint32_t* got_nof_data)
{
  int return_code ;
	int error ;
...
static int sis3150_register_write(HANDLE hDevice, uint32_t addr, uint32_t* data, 
				  uint32_t req_nof_lwords, uint32_t* got_nof_lwords)
{
  unsigned int i ;
  int return_code;
  int nBytes  = 0;
  //UNUSED:int nLWords  = 0;
  
  uint32_t usb_wlength;
  //UNUSED:uint32_t usb_rlength;
  //UNUSED:UINT uiData = 0;
  char cUsbBuf[0x100 + USB_MAX_NOF_BYTES];
...
int EXPORT sis3150Usb_Register_Single_Write(HANDLE usbDevice, uint32_t addr, uint32_t data)
{
  int return_code ;
  uint32_t write_data ;
  uint32_t req_nof_lwords, dma_put_nof_lwords;
...
int EXPORT sis3150Usb_Register_Dma_Write(HANDLE usbDevice, uint32_t addr, 
                            uint32_t* dmabufs, uint32_t req_nof_data, uint32_t* put_nof_data)
{
...
static int sis3150_vmebus_read(HANDLE hDevice, uint32_t addr, uint32_t vme_am, uint32_t vme_size, uint32_t fifo_mode,
			       uint32_t* data, uint32_t req_nof_bytes, uint32_t* got_nof_bytes)
{
  int return_code ;
  unsigned int nBytes  = 0;
  //UNUSED:unsigned int nLWords  = 0;
  //UNUSED:unsigned int i ;
...
int EXPORT sis3150Usb_Vme_Single_Read(HANDLE usbDevice, uint32_t addr, uint32_t am, uint32_t size, uint32_t* data)
{
  int return_code ;
  int ret_code ;
  uint32_t req_nof_bytes, dma_got_no_of_bytes;
  uint32_t reg_data, got_length ;
  ...
  //printf("sis3150Usb_Vme_Single_Read: 0x%lx 0x%d %d 0x%lx\n",addr,am,size,(size_t)data);
...
int EXPORT sis3150Usb_Vme_Dma_Read(HANDLE usbDevice, uint32_t addr, uint32_t am, 
				   uint32_t size, uint32_t fifo_mode,
                            uint32_t* dmabufs, uint32_t req_nof_data, uint32_t* got_nof_data)
{
...
// end mod 2010-07-20
	...
	do {
        ...
        if(error)
        {
            printf("sis3150Usb_Vme_Dma_Read::sis3150_vmebus_read error(%d)\n",error);
        }
      // begin mod 2010-07-20
	      if (size == 2) {
...
static int sis3150_vmebus_write(HANDLE hDevice, uint32_t addr, uint32_t vme_am, uint32_t vme_size, uint32_t fifo_mode,
				uint32_t* data, uint32_t req_nof_bytes, uint32_t* put_nof_bytes)
{
  int return_code ;
  int nBytes  = 0;
  //UNUSED:int nLWords  = 0;
  ...
  /*  end modification 2015-03-16  (1.2-006) , sis3153 VME D16/D8 cycles */
	...
	long_cUsbBuf_ptr =  (uint32_t*) (cUsbBuf + 12);
...
int EXPORT sis3150Usb_Vme_Single_Write(HANDLE usbDevice, uint32_t addr, uint32_t am, uint32_t size, uint32_t data)
{
  int return_code ;
  uint32_t write_data ;
  uint32_t req_nof_bytes, dma_put_nof_bytes;
...
int EXPORT sis3150Usb_Vme_Dma_Write(HANDLE usbDevice, uint32_t addr, uint32_t am, uint32_t size, uint32_t fifo_mode,
                            uint32_t* dmabufs, uint32_t req_nof_data, uint32_t* put_nof_data)
{
// size 2,4,8 sind nur erlaubt (BLT16,BLT32,MBLT64)
...
int sis3150_tsbus_read(HANDLE hDevice, uint32_t addr, uint32_t* data, 
                      uint32_t req_nof_lwords, uint32_t* got_nof_lwords)
{
  int return_code ;
  unsigned int nBytes  = 0;
...
int EXPORT sis3150Usb_TsBus_Single_Read(HANDLE usbDevice, uint32_t addr, uint32_t* data)
{
  int return_code ;
  uint32_t req_nof_lwords, dma_got_nof_lwords;
...
int EXPORT sis3150Usb_TsBus_Dma_Read(HANDLE usbDevice, uint32_t addr, uint32_t* dmabufs, uint32_t req_nof_data, uint32_t* got_nof_data)
{
  int return_code ;
	int error ;
	unsigned int new_addr ; 
...
int sis3150_tsbus_write(HANDLE hDevice, uint32_t addr, uint32_t* data, 
                      uint32_t req_nof_lwords, uint32_t* got_nof_lwords)
{
  unsigned int i ;
  int return_code;
...
int EXPORT sis3150Usb_TsBus_Single_Write(HANDLE usbDevice, uint32_t addr, uint32_t data)
{
  int return_code ;
  uint32_t write_data ;
  uint32_t req_nof_lwords, dma_put_nof_lwords;
...
int EXPORT sis3150Usb_TsBus_Dma_Write(HANDLE usbDevice, uint32_t addr, 
                            uint32_t* dmabufs, uint32_t req_nof_data, uint32_t* put_nof_data)
{
  unsigned int  fifo_mode ;
