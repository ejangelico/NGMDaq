...
int SIS3316System::CreateDefaultConfig(char const *configname)
{
...
int SIS3316System::readConfigFile(const char* configfile){
    TFile* confFin = TFile::Open(configfile);
    NGMSystemConfiguration* tmpConf = dynamic_cast<NGMSystemConfiguration*>(confFin->Get("NGMSystemConfiguration"));
    if(! tmpConf ){
        LOG<<"Unable to Find NGMSystemConfiguration in file "<<configfile<<ENDM_WARN;
	return -1;
    }
    if(_config)
        delete _config;
...
void SIS3316System::SetInterfaceType(const char* vmeinterfacestr)
{
    _vmeinterfacetype = vmeinterfacestr;
}

int SIS3316System::InitializeSystem()
{
...
void SIS3316System::Default3316Card(sis3316card* vslot)
{
...
int SIS3316System::StartAcquisition()
{
...
int SIS3316System::StartListModeGeneral()
{
    NGMConfigurationParameter* cards = _config->GetSlotParameters()->GetColumn("card");
    sis3316card* firstcard = dynamic_cast<sis3316card*>(cards->GetValueO(0));
    ...
    //Create a local vector of card pointers
    ...
    for(int icard = 0; icard < vcards.size(); icard++)
    {
        vcards[icard]->ResetRunScalars();
    }
	...
	singleCardAtThreshold = false;
		...
		for(int icard = 0; icard < vcards.size(); icard++){
		  if(vcards[icard]->DataThresholdReached()){
		    SysError("StartListModeGeneral","Data Treshold reached for card %d\n",icard);
		    ...
		    break;
                ...
                if(1){
                    ...
                    return_code = vmei->vme_A32D32_read(firstcard->baseaddress + SIS3316_IRQ_CONFIG,&irqconfig);
                    return_code = vmei->vme_A32D32_read(firstcard->baseaddress + SIS3316_ADC_CH1_ACTUAL_SAMPLE_ADDRESS_REG,&actualAddress);
                    return_code = vmei->vme_A32D32_read(firstcard->baseaddress + SIS3316_ACQUISITION_CONTROL_STATUS,&ctlstatus);
                    return_code = vmei->vme_A32D32_read(firstcard->baseaddress + SIS3316_IRQ_CONTROL,&irqstatus);
            ...
            if(poll_counter%1==0)
            {
                ...
                if((_runduration-timeOfLastTemp)>timeToReadTemp)
                {
                    ...
                    LOG<<"SIS3316 Temperature:"<<thisTemp<<" "<<ENDM_INFO;
                    if(thisTemp>maxTemp)
                    {
                        LOG<<"SIS3316 Temperature Exceeding maximum of 52C < "<<thisTemp<<ENDM_FATAL;
                        _requestStop = true;
                    }
                }
            }
        ...
        } while ((data_rd & 0xFE) == 0x00000 && !_requestStop && poll_counter<100 && !singleCardAtThreshold);
    ...
    for(int icard = 0; icard < cards->GetEntries(); icard++)
    {
        ...
        if(false){ //Parse data
            ...
            for(int iadc = 0; iadc<SIS3316_ADCGROUP_PER_CARD; iadc++)
            {
                ...
                {
                    card->FetchDataForBlock(iadc);
                    ParseADCBlockData(card,iadc);
                }else{
                    for(int ic = 0; ic<SIS3316_CHANNELS_PER_ADCGROUP; ic++)
                    {
                        int ichan=iadc*SIS3316_CHANNELS_PER_ADCGROUP+ic;
                        card->FetchDataForChannel(ichan);
                        ParseSingleChannelData(card, ichan);
                    }
                }
...
int SIS3316System::ParseADCBlockData(sis3316card* vcard, int iblock)
{
    //These methods are implemented in NGMSIS3316RawReader
    return 0;
}

int SIS3316System::ParseSingleChannelData(sis3316card* vcard, int ichan)
{
    //These methods are implemented in NGMSIS3316RawReader
    return 0;
}

int SIS3316System::StopAcquisition()
{
    return 0;
}
int SIS3316System::RequestAcquisitionStop()
{
    _requestStop = true;
    return 0;
}
void SIS3316System::PlotAdcDistribution(int slot, int channel)
{
    return;
}
int SIS3316System::writePacket(int packetlength, unsigned int *data)
{
    return 0;
}
int SIS3316System::WriteGlobalToTable()
{
    return 0;
}
int SIS3316System::WriteTableToGlobal()
{
    return 0;
}

int SIS3316System::WriteSpillToFile()
{
...
void SIS3316System::closeRawOutputFile()
{
    //Write Raw Trailer
    size_t written;
    if(_rawFilePointer)
...
void SIS3316System::openRawOutputFile()
{
