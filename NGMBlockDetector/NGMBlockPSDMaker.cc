...
NGMBlockPSDMaker::NGMBlockPSDMaker()
{
  InitCommon();
}

NGMBlockPSDMaker::NGMBlockPSDMaker(const char* name, const char* title)
...
bool  NGMBlockPSDMaker::init()
{
  return true;
}

bool  NGMBlockPSDMaker::process(const TObject& tData)
{
...
bool  NGMBlockPSDMaker::finish()
{
  return true;
}
void  NGMBlockPSDMaker::LaunchDisplayCanvas()
{
    TString cName;
    ...
    int ipad = 0;
    for (hiter = _blockPSD.begin(); hiter!=_blockPSD.end(); hiter++) {
        cCanvas->cd(ipad++);
        hiter->second->Draw();
    }
    cCanvas->Modified();
...
void  NGMBlockPSDMaker::ResetHistograms()
{
  std::map<int,TH2*>::iterator hiter;
  for (hiter = _blockPSD.begin(); hiter!=_blockPSD.end(); hiter++) {
      TString hname;
      hname.Form("%s_PSD_%s",GetName(),_bmap.GetDetectorName(hiter->first).c_str());
      hiter->second->SetNameTitle(hname.Data(),hname.Data());
      hiter->second->Reset();
  }
  for (hiter = _blockPSDvsE.begin(); hiter!=_blockPSDvsE.end(); hiter++) {
      TString hname;
      hname.Form("%s_PSDvsE_%s",GetName(),_bmap.GetDetectorName(hiter->first).c_str());
      hiter->second->SetNameTitle(hname.Data(),hname.Data());
    hiter->second->Reset();
  }
}

void NGMBlockPSDMaker::initBlockHist(int hwchan)
{
    int blockSeq = 0;
    
    if(_bmap.IsBlock(hwchan))
    {
        blockSeq = _bmap.GetBlockSequence(hwchan);
    }else{
        blockSeq = hwchan;
    }
    const double maxE = 8000.0;
    const int ebins = 1000.0;
    const int adcbins = 1000.0;
    
    if(!_blockPSD.count(blockSeq)) {
        TString hname;
        hname.Form("%s_PSD_%s",GetName(),partID->getChannelName(hwchan).Data());
        ...
        _blockPSD[blockSeq] = htmp;
    }
    
    if(!_blockPSDvsE.count(blockSeq)) {
        TString hname;
        hname.Form("%s_PSDvsE_%s",GetName(),partID->getChannelName(hwchan).Data());
        //Find Histogram in current folder
        TH2* htmp = dynamic_cast<TH2*>( GetParentFolder()->FindObject(hname));
        if(!htmp)
        {
        ...
        _blockPSDvsE[blockSeq] = htmp;
        LOG<<"Adding histogram " << hname.Data() << ENDM_INFO;
    }
    for(int ipgrp = 0; ipgrp<5; ipgrp++){
        if(!_blockPSDp[ipgrp].count(blockSeq)) {
            TString hname;
            hname.Form("%s_PSD%d_%s",GetName(),ipgrp,partID->getChannelName(hwchan).Data());
            //Find Histogram in current folder
            TH2* htmp = dynamic_cast<TH2*>( GetParentFolder()->FindObject(hname));
            if(!htmp)
            {
            ...
            _blockPSDp[ipgrp][blockSeq] = htmp;
        }
        
        if(!_blockPSDvsEp[ipgrp].count(blockSeq)) {
            TString hname;
            hname.Form("%s_PSDvsE%d_%s",GetName(),ipgrp,partID->getChannelName(hwchan).Data());
            //Find Histogram in current folder
            TH2* htmp = dynamic_cast<TH2*>( GetParentFolder()->FindObject(hname));
            if(!htmp)
            {
            ...
            htmp->SetBins(ebins,0,maxE,1000,_minPSD,_maxPSD);
            _blockPSDvsEp[ipgrp][blockSeq] = htmp;
            LOG<<"Adding histogram " << hname.Data() << ENDM_INFO;
        }
    }
...
void NGMBlockPSDMaker::Print(Option_t* option) const {}

bool NGMBlockPSDMaker::processConfiguration(const NGMSystemConfiguration* conf)
{
...
void NGMBlockPSDMaker::InitCommon()
{
  _maxADC = 4000;
  _minPSD = 0.5;
  _maxPSD = 1.5;
...
bool NGMBlockPSDMaker::processHit(const NGMHit* tHit)
{
  ...
  if(_bmap.IsBlock(hwchan))
  {
    blockSeq = _bmap.GetBlockSequence(hwchan);
      ...
      return false;//blockSeq = hwchan;
  }
    
  if(!_blockPSD.count(blockSeq)) initBlockHist(hwchan);
    
  TH2* hpsd = _blockPSD[blockSeq];
  hpsd->Fill(tHit->GetPulseHeight(),tHit->GetPSD());
  
  TH2* hpsde = _blockPSDvsE[blockSeq];
  hpsde->Fill(tHit->GetEnergy(),tHit->GetPSD());
  if(_bmap.IsBlock(hwchan))
  {
      _blockPSDp[0][blockSeq]->Fill(tHit->GetPulseHeight(),tHit->GetGate(0)/1000.0);
      _blockPSDvsEp[0][blockSeq]->Fill(tHit->GetEnergy(),tHit->GetGate(0)/1000.0);
      _blockPSDp[1][blockSeq]->Fill(tHit->GetPulseHeight(),tHit->GetGate(1)/1000.0);
      _blockPSDvsEp[1][blockSeq]->Fill(tHit->GetEnergy(),tHit->GetGate(1)/1000.0);
      _blockPSDp[2][blockSeq]->Fill(tHit->GetPulseHeight(),tHit->GetGate(2)/1000.0);
      _blockPSDvsEp[2][blockSeq]->Fill(tHit->GetEnergy(),tHit->GetGate(2)/1000.0);
      _blockPSDp[3][blockSeq]->Fill(tHit->GetPulseHeight(),tHit->GetGate(3)/1000.0);
      _blockPSDvsEp[3][blockSeq]->Fill(tHit->GetEnergy(),tHit->GetGate(3)/1000.0);
  }
  return true;
}


bool NGMBlockPSDMaker::fitNeutronGammaBand(const char* detectorPrefix, double minx,double maxx, int nbins,bool ADCorE)
{
...
//  //Lets manually split the last bin into two bins
//  xval.Set(xval.GetSize()+1);
//  xval[xval.GetSize()-1]=xval[xval.GetSize()-2];
//  xval[xval.GetSize()-2]=(xval[xval.GetSize()-3]*0.5+xval[xval.GetSize()-1])*0.5;
//    for(int i2 = xval.GetSize()-3;i2<xval.GetSize();i2++)
//        std::cout<<"Split "<<i2<<" : "<<xval[i2]<<std::endl;
    
 for(int i2 = 0; i2<xval.GetSize();i2++)
     std::cout<<"Final Bins: "<<i2<<" : "<<xval[i2]<<std::endl;
  ...
  NGMBlockPSD* psd = new NGMBlockPSD();
...
bool NGMBlockPSDMaker::CalculateFOM1(TH2* hNScore)
{
...
TSpline3* NGMBlockPSDMaker::MakeCubicSpline(const char* name, TH1* hX, TH1* hY)
{
  TString tgname(name);
  tgname+="_tg";
...
ClassImp(NGMBlockPSD)
NGMBlockPSD::NGMBlockPSD()
{
    adcore=0;
}
void NGMBlockPSD::SetCalibration(TSpline3* gMean, TSpline3* gSig, TSpline3* nMean, TSpline3* nSig)
{
  _gMean = *gMean;
  _gSig = *gSig;
  _nMean = *nMean;
...
double NGMBlockPSD::GetNSigma(double psd,double e) const
{
    return (psd - _nMean.Eval(e))/_nSig.Eval(e) ;
}

double NGMBlockPSD::GetGSigma(double psd,double e) const
{
    return (psd - _gMean.Eval(e))/_gSig.Eval(e) ;
}


double NGMBlockPSD::GetNSigma(const NGMHit* hit) const
{
  if(adcore==0)
...
double NGMBlockPSD::GetGSigma(const NGMHit* hit) const
{
  if(adcore==0)
...
TCutG* NGMBlockPSD::GetNeutronCut(double emin, double emax, double nsig, double gammaSigExclusion, int Npx) const
{
    ...
    std::cout<<"First non zero bin at "<<iE<<std::endl;
    int firstiE = iE;
