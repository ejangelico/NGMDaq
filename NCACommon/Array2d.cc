...
ClassImp(nca::Array2d)

namespace nca {
    
    
Array2d::Array2d(Int_t nX, Int_t nY){
...
Array2d * Array2d::Clone(Bool_t bZeroValues){
    
    Array2d * returnArray = new Array2d(numX,numY);
...
void Array2d::Reset(Int_t nX, Int_t nY,
                      Double_t lowX, Double_t hiX,
                      Double_t lowY, Double_t hiY){
...
void Array2d::SetRange(Double_t lowLimit, Double_t hiLimit, Bool_t bAxis){
    ...
    else printf("Error in Array2d::SetRange() - lower limit (%f) larger than higher limit (%f)\n",lowLimit,hiLimit);
}

void Array2d::SetBinSize(Double_t sizeVal, Bool_t bAxis, Double_t lowLimit){
    ...
    else printf("Error in Array2d::SetBinSize() - bin size requested is negative (%f)\n",sizeVal);

}

Int_t Array2d::globalIndex(Int_t ix, Int_t iy){
    Int_t index = iy*numX + ix;
    if(index<0 || index >= num || ix > numX || ix<0 || iy > numY || iy<0){
        printf("ERROR in Array2d::globalIndex(): Array index out of bounds: %d, %d (%d). Upper value is %d (%d x %d).\n\n",
...
void Array2d::IndexXY(Int_t index, Int_t &binX, Int_t &binY){
    //    Int_t index = iy*numX + ix;
    binX = index%numX;
    binY = (index-binX)/numX;
}

Double_t Array2d::val(Int_t ix, Int_t iy){
    return theArray[globalIndex(ix,iy)];
}

Double_t Array2d::CycledVal(Int_t ix, Int_t iy){
    
    return val(GetCycledIndexX(ix),GetCycledIndexY(iy));
}

Int_t Array2d::GetCycledIndex(Int_t i0, Bool_t bAxis){
...
Int_t Array2d::GetCycledIndex(Double_t d0, Bool_t bAxis, Double_t &cyclesMoved){
...
void Array2d::SetSumValue(Double_t newVal){
    arraySum = newVal;
    bRandomSetup = false;
}

void Array2d::IncrementSumValue(Double_t incVal){
    arraySum += incVal;
    bRandomSetup = false;
}

void Array2d::SetVal(Int_t ix, Int_t iy, Double_t dVal){
    Double_t prevVal = theArray[globalIndex(ix, iy)];
    theArray[globalIndex(ix,iy)] = dVal;
    IncrementSumValue(dVal-prevVal);
}

void Array2d::AddVal(Int_t ix, Int_t iy, Double_t dVal){
    theArray[globalIndex(ix,iy)] += dVal;
    IncrementSumValue(dVal);
}

void Array2d::ClearValues(){
...
void Array2d::SetUniform(Double_t newVal){
    for(Int_t i=0; i<num && theArray; i++){
        theArray[i] = newVal;
...
void Array2d::CopyValues(Array2d *arrayToCopy){
    ...
    if(theArray){
        ...
        printf("ERROR in Array2d::CopyValues(): Trying to copy array values without instantiating array first\n");
    }
}

void Array2d::AddArrays(Array2d *array1, Array2d *array2, Double_t scale1, Double_t scale2){
    ...
    if(array1 && array2){
        ...
        else printf("ERROR in Array2d::AddArrays(): Arrays are not the same length.\n");
        
    }
    else printf("ERROR in Array2d::AddArrays(): Trying to add arrays but at least one pointer is null\n");
    
}

void Array2d::DivideArrays(Array2d *array1, Array2d *array2){
    ...
    if(array1 && array2){
        ...
        else printf("ERROR in Array2d::DivideArrays(): Arrays are not the same length.\n");
        
    }
    else printf("ERROR in Array2d::DivideArrays(): Trying to add arrays but at least one pointer is null\n");
    
}

void Array2d::MultiplyArrays(Array2d *array1, Array2d *array2, Double_t scale1, Double_t scale2){
//void Array2d::MultiplyArrays(Array2d *array1, Array2d *array2, Double_t scale1, Double_t scale2){
...
void Array2d::ScaleArrayValues(Double_t scaleFactor){
...
void Array2d::Fill(Double_t xVal, Double_t yVal, Double_t incVal){
 
    if(!bBinX || !bBinY){
        printf("\tWarning in Array2d::Fill() - array does not have limits defined.\n");
        return;
    }
...
TH2F * Array2d::GetArrayHistogram(const Char_t *histName, Bool_t bAbs){
 
    if(::ROOT::GetROOT()->FindObject(histName)){
        delete ::ROOT::GetROOT()->FindObject(histName);
    }
...
void Array2d::GetMatrixFromArray(TMatrixD *theMatrix){
    
    theMatrix->SetMatrixArray(theArray,"F");
    
}

Array2d * Array2d::GetArray(TH2 *histogram){
 
    Array2d *returnArray = 0;
    
    if(histogram){
        returnArray = new Array2d(histogram->GetNbinsX(),histogram->GetNbinsY(),
...
Array2d * Array2d::GetContentArray(Int_t bins, Double_t xLo, Double_t xHi){
    
    Array2d *returnArray = new Array2d(bins,1,xLo,xHi,-0.5,0.5);
...
Array2d * Array2d::GetContentArray(Int_t bins, Double_t axisPaddingFraction){
...
Bool_t Array2d::GetArrayIndex(Double_t testVal, Bool_t bAxis, Int_t &rBin){
...
Double_t Array2d::GetBinPosition(Int_t iBin, Bool_t bAxis, Bool_t bAllowOutsideBounds){
...
Double_t Array2d::GetQuantity(Int_t iCase, Double_t relVal){
    ...
    if(iCase==0) return max;
        ...
        if(TMath::Abs(mean-relVal) > 1e-9){
            printf("\tWarning in Array2d::GetQuantity(3,%f) - Std. Dev. calculated using a mean of %f, but calculated mean is %f\n",relVal,relVal,mean);
        }
        return stdDev;
    }
    else return 0.0;
...
void Array2d::GetMaximumIndexXY(Int_t &maxIndexX, Int_t &maxIndexY){
...
Bool_t Array2d::RehistogramArrayByInterpolation(Array2d *arrayOld, Double_t acceptanceRadius){
    
    if(!arrayOld->GetBinningStatus()){
        printf("Error in Array2d::RehistogramArrayByArea() - original array does not have absolute bin limits set up\n");
        return false;
    }
    ...
    if(acceptanceRadius > 0.0){
        if( (GetLowX() + GetHighX() > 0.001) || (GetLowY() + GetHighY() > 0.001) ){
            printf("Warning in Array2d::RehistogramArrayByArea() - radial cut requested, but method\n");
...
Bool_t Array2d::RehistogramArrayByArea(Array2d *arrayOld, Bool_t bCyclic, Double_t acceptanceRadius){
    ...
    if(!GetBinningStatus()){
        printf("Error in Array2d::RehistogramArrayByArea() - new array does not have absolute bin limits set up\n");
        return false;
    }
    ...
    Array2d indexScaleFactors = Array2d(numX,numY);
    ...
    for(Int_t iix=0; iix<numX+extraBins[0]+extraBins[1]; iix++){
        ...
        if(!bLoX || !bHiX){
            printf("Warning in Array2d::RehistogramArrayByArea() - x bin not found\n");
            continue;
        }
        ...
        for(Int_t iiy=0; iiy<numY+extraBins[2]+extraBins[3]; iiy++){
            ...
            if(!bLoY || !bHiY){
                printf("Warning in Array2d::RehistogramArrayByArea() - y bin not found\n");
                continue;
            }
            ...
            if(xBinToInc < 0 || xBinToInc >= numX || yBinToInc < 0 || yBinToInc >= numY){
                printf("Warning in Array2d::RehistogramArrayByArea() - cyclic index value not determined correctly.\n");
...
Int_t Array2d::GetRebinnedContribution(Int_t theXbin, Int_t theYbin, Array2d *arrayOld,
    ...
    for(Int_t ix=binLoX-cyclesLoX*numX; ix<=binHiX-cyclesHiX*numX; ix++){
        for(Int_t iy=binLoY-cyclesLoY*numY; iy<=binHiY-cyclesHiY*numY; iy++){
            ...
            if(totPixContributing<MAX_REBIN_MAP_PIX){
                ...
                printf("\tError in Array2d::GetRebinnedContribution() - bin %d, %d of original array overlaps with too many rebinned bins\n",theXbin,theYbin);
...
Double_t Array2d::GetFractionalCoverage(Int_t iThisX, Int_t iThisY, Array2d *arrayOld, Int_t iOldX, Int_t iOldY){
...
void Array2d::GetRandomBoundaries(){
    
    Double_t absInt = AbsIntegral();
    if(absInt < 1e-9){
        printf("\tWarning in Array2d::GetRandomBoundaries() - cannot set up random boundaries for empty array\n");
        bRandomSetup = false;
        return;
    }
...
Bool_t Array2d::GetRandom(Double_t &xRand, Double_t &yRand){
    
    if(!bRandomSetup) GetRandomBoundaries();
    
    if(!bRandomSetup){
        printf("\tWarning in Array2d::GetRandom() - problem setting up random boundaries\n");
        return false;
    }
