...
READ_SAMPLES_ALL_AT_ONCE = True
READ_WHOLE_FILE = True

word_offset = 0     # Current offset into file - 32 bit words
file_content = None

# Read word method so can instrument the timing
def read_word(infile):
    if READ_WHOLE_FILE:
        global word_offset
        global file_content
        word = file_content[word_offset]
        word_offset += 1
    else:
        word = struct.unpack("<I", infile.read(4))[0]
    return word

def read_words(infile, number_of_words):
    global word_offset
    if READ_WHOLE_FILE:
        global word_offset
        global file_content
        words = file_content[slice(word_offset, word_offset + number_of_words)]
        word_offset += number_of_words
    else:
        number_of_bytes = 4*number_of_words   # huh?
        format = '<' + str(2*number_of_words) + 'H'
        #print('Size of data block: %d' % struct.calcsize(format))
        words = struct.unpack(format, infile.read(number_of_bytes))
    return words

# More instrumentation
def append_sample(list, word):
    list.append(word & 0x0000ffff)
    list.append((word >> 16) & 0x0000ffff)
...
def read_file(filename, max_num_of_spills=1):
    if READ_WHOLE_FILE:
        ...
        with open(_args.file, mode='rb') as file:  # b is important -> binary
            ...
            file_content = np.fromfile(file, dtype=np.uint32)  # Read as int16s
        run_header, spills_list, words_read, spill_counter = really_read_file(None, max_num_of_spills)
    else:
        with open(filename, 'rb') as infile:
            run_header, spills_list, words_read, spill_counter = really_read_file(infile, max_num_of_spills)
    return run_header, spills_list, words_read, spill_counter

def really_read_file(infile, max_num_of_spills):
    ...
    for i in range(100):
        run_header.append(hex(read_word(infile)))

    first_word_of_spillhdr = hex(read_word(infile))
...
def read_spill(infile, first_entry_of_spillhdr):
    ...
    for i in range(9):
        spill_header.append(hex(read_word(infile)))
    ...
    while True:
        ...
        if (this_card_id != previous_card_id) and (hdrid_temp & 0xff0000 == 0):
            ...
            # print('READING NEXT CARD: %d' % this_card_id)
            phdrid = []
            phdrid.append(hdrid_temp)
            phdrid.append(read_word(infile))
...
def read_channel(infile):
    ...
    for i in range(6):
        trigger_stat_spill.append(hex(read_word(infile)))
    ...
    data_buffer_size = read_word(infile)
...
def read_event(infile):
    ...
    if READ_SAMPLES_ALL_AT_ONCE:
        num_raw_samples = event['num_raw_samples']
        samples = read_words(infile, num_raw_samples)
        sample_list = list(samples)     # TODO: Don't need this step if we are reading the whole file at once
        # Flip each pair of samples - See Struck doc
        for i in range(num_raw_samples):
            if READ_WHOLE_FILE:
                # See https://stackoverflow.com/questions/27506474/how-to-byte-swap-a-32-bit-integer-in-python
                # sample_list[i] = (((sample_list[i] << 16) & 0xFFFF0000) |
                #                   ((sample_list[i] >> 16) & 0x0000FFFF))
                pass
            else:
                temp_sample = sample_list[i]
                sample_list[i] = sample_list[i+1]
                sample_list[i+1] = temp_sample
        event['samples'] = sample_list
        bytes_read += 4*num_raw_samples
    else:
        event['samples'] = []
        for i in range(event['num_raw_samples']):
            ...
            word = read_word(infile)
            bytes_read += 4
            append_sample(event['samples'], word)
